<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="ISO-8859-1">
		<title>Welcome on Amazerific</title>
	</head>
	<body>
		<header>
			<h1>Amazerific</h1>
		</header>
		<main>
			<p>Sito di e-commerce nato 5 secondi fa e già in vetta alle classfiche</p>
			<br>
			<p>Scegli l'operazione che vuoi andare a fare</p>
			<ul>
				<li><a href="login_page">Fai il login</a></li>
				<li><a href="#">Registrati</a>
				<li><a href="#">Visualizza prodotti disponibili</a></li>
				<li><a href="#">Aggiungi al carrello</a></li>
				<li><a href="#">Vai al carrello</a>
				<li><a href="#">Dettaglio oridini</a></li>
				<li><a href="#">Fai logout</a></li>
			</ul>
		</main>
		<footer>
		</footer>
	</body>
</html>